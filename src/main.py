...
class LoadTester:
    ...
    def generate_report(self) -> TestResult:
        return TestResult(
            ...
            average_response_time=sum(self.results["response_times"]) / len(self.results["response_times"]) if self.results["response_times"] else 0,
            status_code_distribution={str(k): v for k, v in dict(Counter(self.results["status_codes"])).items()},
